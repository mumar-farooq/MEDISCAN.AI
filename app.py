...
<div class="header-banner">
    ...
    <div class="logo-badge">MEDI SCAN AI v2.5</div>
</div>
...
def render_range_meter(test_name, value, sex="male"):
    ...
    <div style="margin-top: 12px; margin-bottom: 4px;">
        <div style="display: flex; justify-content: space-between; font-size: 11px; color: #52616b; font-family: 'Inter', sans-serif; margin-bottom: 6px;">
            <span>Ref: {low} - {high} {ref['unit']}</span>
            <span>Extracted: <b>{value}</b></span>
        </div>
        <div style="position: relative; width: 100%; height: 8px; background: #e0f0fc; border-radius: 9999px; overflow: hidden;">
            <div style="position: absolute; left: {norm_start}%; width: {norm_end - norm_start}%; height: 100%; background: #2e7d5b; opacity: 0.85;"></div>
            <div style="position: absolute; left: 0%; width: {norm_start}%; height: 100%; background: #c1c7d2; opacity: 0.5;"></div>
            <div style="position: absolute; left: {norm_end}%; width: {100 - norm_end}%; height: 100%; background: #ffdad6; opacity: 0.7;"></div>
        </div>
        <div style="position: relative; width: 100%; height: 14px; margin-top: -11px;">
            <div style="position: absolute; left: calc({pct}% - 7px); top: 0px; width: 14px; height: 14px; background: #005087; border: 2px solid #ffffff; border-radius: 50%; box-shadow: 0 1px 4px rgba(0,0,0,0.3);" title="{value} {ref['unit']}"></div>
        </div>
    </div>
    """
...
with tab_ocr:
    ...
    if uploaded_file:
            ...
            with cols[i]:
                ...
                <div class="clarity-card">
                    <div class="clarity-card-header">
                        <span class="clarity-title">{names[name]}</span>
                        {chip_html}
                    </div>
        ...
        st.markdown(f"""<div class="clarity-card">{report_text}</div>""", unsafe_allow_html=True)
        st.download_button(
