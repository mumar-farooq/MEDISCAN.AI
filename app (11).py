...
h1, h2, h3, h4, h5, h6 {
    font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Manrope, Roboto, Arial, sans-serif !important;
...
/* Target the uploader's button by tag, not by a guessed data-testid,
   so styling still applies even if the internal test-id changes between
   Streamlit versions. Scoped to inside the dropzone only. */
...
/* Kill any absolutely-positioned duplicate label some Streamlit builds
   render for accessibility/drag-state, which is what causes the
   doubled "uploadUpload" look when it overlaps the visible label. */
...
/* ---- Guard against Material Symbols icon font failing to load ----
   When the icon font can't load, the browser prints the ligature name
   (e.g. "keyboard_double_arrow_left") as literal text. These elements
   are icon-only controls, so if the glyph fails we hide the text
   instead of showing the raw ligature name. */
